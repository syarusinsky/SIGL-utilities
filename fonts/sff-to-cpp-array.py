# A simple script to turn a sff font file into an initialized array of bytes in the c standard
...
if len(sys.argv) == 1:
    print( "ERROR: you need to specify the name of the sff file to convert" )
    quit()
else:
...
# open files
try:
    sff_file = open( font_name + ".sff", 'rb' )
except IOError:
...
sff_byte = sff_file.read( 1 )
while sff_byte:
    byte_string = format( int.from_bytes(sff_byte, "big"), '#010b')
    sff_byte = sff_file.read( 1 )
    if sff_byte:
